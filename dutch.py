...
if __name__ == '__main__':
    ...
    seeds = [1000]
    #sizes_samples = [22000,18500] #max possiveis
    sizes_samples = [1000,1000]
    values_sens = [['adult','notAdult'],[1,2]]
    names_sens =['age','sex']
    ...
    for i in seeds:
        ...
        for j in range(len(names_sens)):
            ...
            DIRETORIA_EXP = "exp_dutch_differentSplit_seed"+str(SEED)+"_"+str(SENSIVEL_NAME)+"_size"+str(SAMPLE_SIZE)
            ...
            dataset = pd.read_csv('data/dutchTransformed.data', sep=';', na_values=['?', ' ?'], header=0, names=features + [TARGET_NAME])
