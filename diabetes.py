...
if __name__ == '__main__':
    ...
    seeds = [1000]
    #sizes_samples = [13000,40000,20000] #max possiveis
    sizes_samples = [1000,1000,1000]
    values_sens = [['notAdult','adult'],['Female','Male'],['White','notWhite']]
    ...
    for i in seeds:
        ...
        for j in range(len(names_sens)):
            ...
            DIRETORIA_EXP = "exp_diabetes_differentSplit_seed"+str(SEED)+"_"+str(SENSIVEL_NAME)+"_size"+str(SAMPLE_SIZE)
            ...
            dataset = pd.read_csv('data/diabetesTransformed.data', sep=';', na_values=['?', ' ?'], header=0, names=features + [TARGET_NAME])
