...
DIRETORIA_EXP = 'exp_creditCard_seed2137_age_semRoc_200'
# [majority, minority]
#SENSIVEL_VALUES = [2, 1]
...
if __name__ == '__main__':
    ...
    for i in seeds:
        ...
        for j in range(len(names_sens)):
            ...
            DIRETORIA_EXP = "exp_creditCard_differentSplit_seed"+str(SEED)+"_"+str(SENSIVEL_NAME)+"_size"+str(SAMPLE_SIZE)
            ...
            dataset = pd.read_csv('data/default of credit card clients.csv', sep=';', na_values=['?', ' ?'], header=1, names=features + [TARGET_NAME])
            ...
            conditions = [
            (dataset['AGE'] < 25),
            (dataset['AGE'] >= 25) & (dataset['AGE'] < 60),
            (dataset['AGE'] >= 60)
            ]

            choices = ['notAdult', 'adult', 'notAdult']

            # Aplicar as condições para criar a nova coluna 'AGE'
            dataset['AGE'] = np.select(conditions, choices, default='unknown')
