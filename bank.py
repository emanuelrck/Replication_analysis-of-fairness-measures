...
if __name__ == '__main__':
    ...
    for i in seeds:
        ...
        for j in range(len(names_sens)):
            ...
            DIRETORIA_EXP = "exp_bank_differentSplit_seed"+str(SEED)+"_"+str(SENSIVEL_NAME)+"_size"+str(SAMPLE_SIZE)
            ...
            dataset = pd.read_csv('data/bank-full.csv', sep=';', na_values=['?', ' ?'], header=0, names=features + [TARGET_NAME])
            # Verificar a transformação
            #print(dataset['race'].value_counts())
            # ------------------------------------------Transformar o atributo 'race'
            dataset['age'] = dataset['age'].apply(lambda x: 'notAdult' if x < 25 else ('adult' if x < 60 else 'notAdult'))
            # Verificar a transformação
            #print(dataset['race'].value_counts())
