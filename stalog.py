...
DIRETORIA_EXP = 'exp_stalog_seed2137_sex'
# [majority, minority]
SENSIVEL_VALUES = ['male', 'female']
SENSIVEL_NAME = 'sex9' # tentar fazer para race
#SAMPLE_SIZE = 2300 #2300 para sexo original era 1100

#SENSIVEL_VALUES = ['adult','notAdult']
#SENSIVEL_NAME = 'Age13'
SAMPLE_SIZE = 200
# ver se funciona
TARGET_VALUES = [1, 2]
TARGET_NAME = 'classlabel21'
...
if __name__ == '__main__':
    ...
    seeds = [1000]
    sizes_samples = [200,150]
    values_sens = [['male', 'female'],['adult','young']]
    names_sens =['sex9','Age13']
    ...
    for i in seeds:
        ...
        for j in range(len(names_sens)):
            ...
            DIRETORIA_EXP = "exp_stalog_differentSplit_seed"+str(SEED)+"_"+str(SENSIVEL_NAME)+"_size"+str(SAMPLE_SIZE)
            ...
            dataset = pd.read_csv('data/german.data', sep=',', na_values=['?', ' ?'], header=1, names=features + [TARGET_NAME])
            ...
            choices_age13 = ['young', 'adult']
            dataset['Age13'] = np.select(conditions_age13, choices_age13, default='unknown')

            # Condições para 'Duration2'
            conditions_duration2 = [
                (dataset['Duration2'] <= 6),
                (dataset['Duration2'] >= 7) & (dataset['Duration2'] < 12),
                (dataset['Duration2'] >= 12)
            ]
            choices_duration2 = ['short', 'medium', 'high']
            dataset['Duration2'] = np.select(conditions_duration2, choices_duration2, default='unknown')

            # Condições para 'Creditamount5'
            conditions_creditamount5 = [
                (dataset['Creditamount5'] <= 2000),
                (dataset['Creditamount5'] > 2000) & (dataset['Creditamount5'] < 5000),
                (dataset['Creditamount5'] >= 5000)
            ]
            choices_creditamount5 = ['short', 'medium', 'high']
            dataset['Creditamount5'] = np.select(conditions_creditamount5, choices_creditamount5, default='unknown')
