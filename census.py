...
SAMPLE_SIZE = 1400
# ver se funciona
TARGET_VALUES = ['less', 'great']
TARGET_NAME = 'taxable.income.amount'
...
if __name__ == '__main__':
    ...
    for i in seeds:
        ...
        for j in range(len(names_sens)):
            ...
            DIRETORIA_EXP = "exp_census_differentSplit_seed"+str(SEED)+"_"+str(SENSIVEL_NAME)+"_size"+str(SAMPLE_SIZE)
            ...
            dataset = pd.read_csv('data/censusTransformed.data', sep=';', na_values=['?', ' ?'], header=0, names=features + [TARGET_NAME])
