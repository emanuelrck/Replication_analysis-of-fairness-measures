...
if __name__ == '__main__':
    ...
    sizes_samples = [1500,800]
    values_sens = [['homem','mulher'],[1,0]]
    names_sens =['male','racetxt']
    ...
    for i in seeds:
        ...
        for j in range(len(names_sens)):
            ...
            DIRETORIA_EXP = "exp_law_differentSplit_seed"+str(SEED)+"_"+str(SENSIVEL_NAME)+"_size"+str(SAMPLE_SIZE)
            ...
            dataset = pd.read_csv('data/lawTransformed.data', sep=';', na_values=['?', ' ?'], header=0, names=features + [TARGET_NAME])
